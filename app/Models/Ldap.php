...
class Ldap extends Model
{
     ...
     */
    public static function findAndBindUserLdap($username, $password, $filterQuerySet = NULL)
    {
		
        $settings = Setting::getSettings();
	
	\Log::debug('Connecting to the Server : '.$settings->ldap_server);
	...
	///ex..   $filterQuerySet = "(&(objectClass=group)(cn=MLS))";
	
	if( $filterQuerySet == NULL )
	{
	...
	{
		$filterQuery =  "(&(objectClass=group)(".$filterQuerySet."))";
		$baseDn =  $settings->ldap_basedn;
    ...
    public static function bindAdminToLdap($connection)
    {
        ...
        if (! $ldapbind = @ldap_bind($connection, $ldap_username, $ldap_pass)) {
            //ricky do nothing //throw new Exception('Could not bind to LDAP: '.ldap_error($connection));
        }
	...
	$ldap_result_description = Setting::getSettings()->ldap_description;
	$ldap_result_memberof = "memberof";
        // Get LDAP user data
        $item = [];
	//dd($ldapattributes['employeenumber'][0]);
        $item['username'] = isset($ldapattributes[$ldap_result_username][0]) ? $ldapattributes[$ldap_result_username][0] : '';
        $item['AccessMapId'] = isset($ldapattributes['employeenumber'][0]) ? $ldapattributes['employeenumber'][0] : '';
	...
	$item['description'] = isset($ldapattributes[$ldap_result_description][0]) ? $ldapattributes[$ldap_result_description][0] : '';
	$item['memberof'] = isset($ldapattributes[$ldap_result_memberof]) ? $ldapattributes[$ldap_result_memberof] : '';
