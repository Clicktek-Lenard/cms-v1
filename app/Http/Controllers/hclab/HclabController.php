...
class HclabController extends Controller
{
    /**
	...
	// 	//echo "i=".$i;
	// 	//echo "<br>";
	// 	$datas = DB::connection('oraPRODh')->table('ORD_HDR')->offset($i)->limit(10000)->get(array('*'));
	// 	echo "Processing";
	// 	foreach($datas as $data)
	// 	{
	// 		echo " -> ";
	// 		DB::connection('hclab')->table('ORD_HDR')->insertGetId([
	// 		'OH_TNO'			=> $data->oh_tno
	...
	// public function HclabORDS()
    // {	
	// 	$i = 0;

	// 	for ($x = 0; $x <= 55; $x++) {

	// 	$datas = DB::connection('oraPRODh')->table('ORD_SPL')->offset($i)->limit(10000)->get(array('*'));
	// 	echo "Processing";

	// 	foreach($datas as $data)
	// {
	// 	echo " -> ";
	// 	echo DB::connection('hclab')->table('OrderSPL')->insertGetId([
	// 	'OS_TNO'			=> $data->os_tno
	// 	,'OS_SPL_TYPE'		=> $data->os_spl_type
	// 	,'OS_SNO'		=> $data->os_sno
	// 	,'OS_SPL_VOLUME'			=> $data->os_spl_volume
	// 	,'OS_SPL_UNIT'			=> $data->os_spl_unit
	// 	,'OS_SPL_TUBE'			=> $data->os_spl_tube
	// 	,'OS_SPL_COLDT'		=> $data->os_spl_coldt
	// 	,'OS_SPL_COLDT_ED'		=> $data->os_spl_coldt_ed
	// 	,'OS_SPL_COLBY'	=> $data->os_spl_colby
	// 	,'OS_SPL_DPDT'	=> $data->os_spl_dpdt
	// 	,'OS_SPL_DPBY'	=> $data->os_spl_dpby
	// 	,'OS_SPL_RTDT'	=> $data->os_spl_rtdt
	// 	,'OS_SPL_RTBY'	=> $data->os_spl_rtby
	// 	,'OS_SPL_RCVDT'	=> $data->os_spl_rcvdt
	// 	,'OS_SPL_RCVD_LS'	=> $data->os_spl_rcvd_ls
	// 	,'OS_SPL_RCVD_WS'			=> $data->os_spl_rcvd_ws
	// 	,'OS_SPL_WS1'			=> $data->os_spl_ws1
	// 	,'OS_SPL_WS2'		=> $data->os_spl_ws2
	// 	,'OS_SPL_WS3'		=> $data->os_spl_ws3
	// 	,'OS_SPL_WS4'		=> $data->os_spl_ws4
	// 	,'OS_SPL_WS5'		=> $data->os_spl_ws5
	// 	,'OS_SPL_COLT_FLAG'		=> $data->os_spl_colt_flag
	// 	,'OS_SPL_DP_FLAG'		=> $data->os_spl_dp_flag
	// 	,'OS_SPL_RT_FLAG'	=> $data->os_spl_rt_flag
	// 	,'OS_SPL_RCVD_FLAG'		=> $data->os_spl_rcvd_flag
	// 	,'OS_SPL_RJ_FLAG'			=> $data->os_spl_rj_flag
	// 	,'OS_SPL_RJ_DT'		=> $data->os_spl_rj_dt
	// 	,'OS_SPL_RJ_CODE'		=> $data->os_spl_rj_code
	// 	,'OS_SPL_RJ_BY'		=> $data->os_spl_rj_by
	// 	,'OS_SPL_CMT'		=> $data->os_spl_cmt
	// 	,'OS_ALT_SPL_TYPE'	=> $data->os_alt_spl_type
	// 	,'OS_ALT_SPL_DESC'		=> $data->os_alt_spl_desc
	// 	,'OS_SITE_CODE'		=> $data->os_site_code
	// 	,'OS_SITE_DESC'		=> $data->os_site_desc
	// 	,'OS_REC_FLAG'	=> $data->os_rec_flag
	// 	,'OS_RACKNO'	=> $data->os_rackno
	// 	,'OS_TUBENO'=> $data->os_tubeno
	// 	,'OS_FLAG1'	=> $data->os_flag1
	// 	,'OS_FLAG2'	=> $data->os_flag2
	// 	,'OS_01_LOGNO'	=> $data->os_01_logno
	// 	,'OS_02_LOGNO'	=> $data->os_02_logno
	// 	,'OS_03_LOGNO'	=> $data->os_03_logno
	// 	,'OS_01D_LOGNO'	=> $data->os_01d_logno
	// 	,'OS_02D_LOGNO'	=> $data->os_02d_logno
	// 	,'OS_03D_LOGNO'		=> $data->os_03d_logno
	// 	,'OS_ARCH_WS_CODE'		=> $data->os_arch_ws_code
	// 	,'OS_ARCH_RACK_ID'	=> $data->os_arch_rack_id
	// 	,'OS_ARCH_POS'	=> $data->os_arch_pos	
	// 	,'OS_ARCH_DT'	=> $data->os_arch_dt	
	// 	,'OS_ARCH_BY'	=> $data->os_arch_by	
	// 	,'OS_ARCH_FLAG'		=> $data->os_arch_flag	
	// 	,'OS_UPDATE_BY'		=> $data->os_update_by
	// 	,'OS_UPDATE_ON'	=> $data->os_update_on	
	// 	,'OS_SPL_PICKUP_FLAG'		=> $data->os_spl_pickup_flag	
	// 	,'OS_SPL_PICKUP_DT'	=> $data->os_spl_pickup_dt
	// 	,'OS_SPL_DELVD_FLAG'		=> $data->os_spl_delvd_flag	
    ...
    public function syncHclabORDH()
    {
		$today = date('Y-m-d');
        $previous_day = date('Y-m-d', strtotime($today . ' -1 day'));
		$datas = DB::connection('oraPRODh')->table('ORD_HDR')->whereDate('OH_UPDATE_ON', '=', $previous_day )->get(array('*'));
		// dd($datas);
		// die();
		echo "Processing";
		echo " -> ";
		foreach($datas as $data){
		echo DB::connection('hclab')->table('ORD_HDR')->updateOrInsert(
		['OH_TNO' 			=> $data->oh_tno],
		['OH_TRX_DT'		=> $data->oh_trx_dt
			,'OH_TYPE'		=> $data->oh_type
			,'OH_ONO'			=> $data->oh_ono
			...
			echo $data->oh_tno." -> Done";
		}
		
	}

	public function syncHclabORDD()
    { 
		// $i = 0;
		...
		$today = date('Y-m-d');
        $previous_day = date('Y-m-d', strtotime($today . ' -1 day'));
		$datas = DB::connection('oraPRODh')->table('ORD_DTL')->whereDate('OD_UPDATE_ON', '=', '2023-07-09')->offset(10000)->limit(10000)->get(array('*'));
	// 	dd($datas);
	// die();
		echo "Processing";
		echo " -> ";
		foreach($datas as $data)
		{
		   ...
		   echo DB::connection('hclab')->table('ORD_DTL')->updateOrInsert(
			[
				'OD_TNO'			=> $data->od_tno
	...
	public function syncHclabORDS()
    {	
		$today = date('Y-m-d');
        $previous_day = date('Y-m-d', strtotime($today . ' -1 day'));
		$datas = DB::connection('oraPRODh')->table('ORD_SPL')->whereDate('OS_UPDATE_ON', '=', '2023-07-09')->get(array('*'));
	// 		dd($datas);
	// die();
		echo "Processing";
		echo " -> ";
		foreach($datas as $data)		
	 	{
			DB::connection('hclab')->table('ORD_SPL')->updateOrInsert (
...
//         $oracleData = DB::connection('oraPRODh')->table('ORD_DTL')
//             ->leftjoin('ORD_HDR', 'ORD_DTL.OD_TNO', '=', 'ORD_HDR.OH_TNO')
//             ->leftjoin('ORD_SPL', 'ORD_HDR.OH_TNO', '=', 'ORD_SPL.OS_TNO')
//             ->limit(1000)
//             ->get(array('*'));
// // dd($oracleData);
// // die();
//         foreach ($oracleData as $record) {
//             DB::connection('hclab')->table('OrderDTL')->updateOrInsert(['OD_TNO' => $record->od_tno], (array)$record);
// 			DB::connection('hclab')->table('OrderHDR')->updateOrInsert(['OH_TNO' => $record->OH_TNO], (array)$record);
// 			DB::connection('hclab')->table('OrderSPL')->updateOrInsert(['OS_TNO' => $record->OS_TNO], (array)$record);
//         }
