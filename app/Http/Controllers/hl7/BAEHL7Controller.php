	...
	public function HL7() // change to push data from CMS to EROS
	{
		...
		if (DB::connection('CMS')->getDatabaseName()) 
		{
			...
			$qTrans =  DB::connection('CMS')->table('Queue')
									...
									->get(array('Queue.Id as QId', 'Queue.Code as QCode','Queue.Date'));
			...
			foreach($qTrans as $que)
			{
				...
				foreach($trans as $trx)
				{
						...
						{
							#Condition for RP#
							$iStatus =  ($hasPendingApproval) ? 212 : 230; // SOAP 230 for vitals first 
							
							if( $itemT->AItemGroup == 'CLINIC')
							{
								$accStatus = BAEHL7Controller::accessionStatus(['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' => $itemT->AItemCode]);
								$iStatus = (count($accStatus) !=0 && ($accStatus[0]->Status != 300 && $accStatus[0]->Status != 850 && $accStatus[0]->Status != 900 )) ? $accStatus[0]->Status : $iStatus;
								
								DB::connection('CMS')->table('AccessionNo')->updateOrInsert
								(
									['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' => $itemT->AItemCode],
									['IdTransaction' => $itemT->Id, 'Type' => $itemT->AItemGroup , 'ItemGroup' => $itemT->AItemGroup, 'ItemSubGroup' => $itemT->AItemSubGroup ,  'Stat' => $itemT->Stat, 'LISCode' => $itemT->ALISCode,   'ItemDescription' => $itemT->AItemDescription, 'Status' => $iStatus ]
								);
								//Update Transaction status based on Accession Status
						...
						foreach($ItemTrans as $itemT)
						{
							$iStatus =  ($hasPendingApproval) ? 212 : 300;
						...
						foreach($PackageTrans as $packT)
						{
							...
							{
								$accStatus = BAEHL7Controller::accessionStatus(['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' => $packT->AItemCode]);
								$iStatus = (count($accStatus) !=0 && ($accStatus[0]->Status != 300 && $accStatus[0]->Status != 850 && $accStatus[0]->Status != 900 )) ? $accStatus[0]->Status : $iStatus;
								
								DB::connection('CMS')->table('AccessionNo')->updateOrInsert
								(
									['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' => $packT->AItemCode], 
									['IdTransaction' => $packT->Id , 'Type' => $packT->AItemGroup , 'ItemGroup' => $packT->AItemGroup , 'ItemSubGroup' => $ItemMasterData->SubGroup ,  'LISCode' => $ItemMasterData->LISCode,  'ItemDescription' => $packT->AItemDescription, 'Status' =>  $iStatus ]
								);
								if(  $standID != $packT->Id)
								{
							...
							else if( $packT->AItemGroup == 'CLINIC')
							{
								$iStatus =  ($hasPendingApproval) ? 212 : 230; // SOAP 230 for vitals first 
								$accStatus = BAEHL7Controller::accessionStatus(['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' => $packT->AItemCode]);
								$iStatus = (count($accStatus) !=0 && ($accStatus[0]->Status != 300 && $accStatus[0]->Status != 850 && $accStatus[0]->Status != 900 )) ? $accStatus[0]->Status : $iStatus;
						...
						foreach($PackageTrans as $packT)
						{
							...
							$iStatus =  ($hasPendingApproval) ? 212 : 300; // from 250 to 300
				...
				//check transaction if they have for payment status
				...
				{	
					...
					$accStatus = BAEHL7Controller::accessionStatus(['IdBU' => $BUCode, 'Date' => $que->Date,  'QueueCode' => $que->QCode , 'IdQueue' => $que->QId]);
					$specimen = 0;
					$completed = 0;
					$forVitals = 0;
					...
					foreach($accStatus as $select)
					{
						...
						else if( $select->Status == '230' ) //for vitals
						{
							$forVitals = 1;
						}
					...
					if( $forVitals == 1 && $specimen == 0 && $completed == 0)
					{
						DB::connection('CMS')->table('Queue')->where('Id', $que->QId)->update(['Status' => 230]);
					}
					else
					{
						DB::connection('CMS')->table('Queue')->where('Id', $que->QId)->update(['Status' => 500]);
					}
...
######## to be separated 	makeAccessionNo	
			echo '<br>Done';
			//BAEHL7Controller::makeAccessionNo();
		}
	}
	...
	public function makeAccessionNo()
	{
		...
		DB::connection('CMS')->beginTransaction();
			...
			$ATrans =  DB::connection('CMS')->table('AccessionNo')
									//->leftJoin('Eros.Patient', 'Queue.IdPatient', '=', 'Eros.Patient.Id')
									->where('AccessionNo.Status', '=','300') // 250  // 311
									...
									->get(array('AccessionNo.IdQueue as QId', 'AccessionNo.QueueCode as QCode'));
			//dd($ATrans);
			foreach($ATrans as $que)
			{
				...
				$gbDocComLab = DB::connection('CMS')->table('AccessionNo')->where('IdQueue', $que->QId)->where('Status','300')->where('Type', 'LIKE', 'LABORATORY')->groupBy('IdDoctor', 'IdCompany')->get(array('*'));
				...
				foreach($gbDocComLab as $gbDC)
				{
					$status =  DB::connection('CMS')->table('AccessionNo')->where('IdQueue', $que->QId)->where('Status','>=','260')->where('Status','<=','650')->where('Type', 'LIKE', 'LABORATORY')
						...
						//->where('ItemSubGroup', $gbDC->ItemSubGroup )
					...
					DB::connection('CMS')->table('AccessionNo')->where('IdQueue', $que->QId)->where('Status','300')->where('Type', 'LIKE', 'LABORATORY')
					->where('IdCompany', $gbDC->IdCompany)->where('IdDoctor', $gbDC->IdDoctor)
					//->where('ItemSubGroup', $gbDC->ItemSubGroup )
					->update(['Status' => 360, 'AccessionNo' => $alphaNew]);
				}
				//checking status from 260 upto 650 
				$status = DB::connection('CMS')->table('AccessionNo')->whereNotNull('AccessionNo')->where('IdQueue', $que->QId)->where('Status','>=','260')->where('Status','<=','900')->where('Type', 'LIKE', 'IMAGING')->get(array('*'));
				if( count($status) != 0 )
				{
					$y = count($status);
				}
				$gbDocComImaging = DB::connection('CMS')->table('AccessionNo')->whereNull('AccessionNo')->where('IdQueue', $que->QId)->where('Status','300')->where('Type', 'LIKE', 'IMAGING')->groupBy('IdDoctor', 'IdCompany', 'IdTransaction','ItemCode')->get(array('*'));
				
				foreach($gbDocComImaging as $gbDC)
				{
					DB::connection('CMS')->table('AccessionNo')->where('IdQueue', $que->QId)->where('Status','300')->where('Type', 'LIKE', 'IMAGING')->where('IdTransaction', $gbDC->IdTransaction)
				...
				DB::connection('CMS')->table('Queue')->where('Id', $que->QId)->update(['Status' => 360]); // 360
			}
	...
	{	
		...
		$qTrans =  DB::connection('CMS')->table('Queue')
					->where('Queue.Date', '=', date('Y-m-d'))
					->where('Queue.Status', '=', 360) //Processing HL7
					->where('Queue.IdBU', 'LIKE', $BUCode)
					//->where('Queue.Id', '=', '34592810')
					->limit(1)
					->orderBy('Queue.Id', 'DESC')
					->get(array('Queue.Id as QId','DateTime', 'Queue.Date', 'Queue.Code as QCode'));
		...
		foreach ($qTrans as $que)
		{
			...
			else if( $specimen == 0  && $forConsultation == 0 &&  $forVitals == 1)
			{
				DB::connection('CMS')->table('Queue')->where('Id', $que->QId)->update(['Status' => 230]);
			}
			...
			foreach($accDatas as $acc)
			{
				...
				DB::connection('CMS')->table('AccessionNo')->where('Status', 360)->where('AccessionNo', '=', $acc->AccessionNo )->update(['Status' => 370]);
	...
	public function cenGetAccessionMapUpdate($iDate = NULL) // re-call 
	{
		//$today =  "2024-09-15%";
		$today =  date('Y-m-d');
		...
		//DB::connection('CMS')->beginTransaction();
		$accessionDatas =  DB::connection('CMS')->table('AccessionNo')
					->where('AccessionNo.Date', '=', $today)
					->where('AccessionNo.Status', '=', 370) // from 300
		...
		foreach ($accessionDatas as $aData)
		{
			...
			{	// get Labno and Update AccessionMap
				$ordHRD = DB::connection('oraCENh')->table('ORD_HDR')->where('OH_ONO', 'LIKE', $aData->AccessionNo)->groupBy('OH_TNO', 'OH_TRX_DT')->get(array('OH_TNO','OH_TRX_DT'));
				if(count($ordHRD) != 0)
				{
					echo "<br>Update Accession Map => ". $ordHRD[0]->oh_tno;
					$hcDate = date('Y-m-d', strtotime($ordHRD[0]->oh_trx_dt));
					DB::connection('CMS')->table('AccessionNo')
					->whereIN('AccessionNo.IdBU', ['BAE'] ) // latest branch as of Sep 13 2024
					->where('AccessionNo.Type', 'LIKE', 'LABORATORY')->where('IdQueue', $aData->IdQueue)->where('AccessionNo', $aData->AccessionNo)->update(['ExamDate' => $hcDate ,'AccessionMap' => $ordHRD[0]->oh_tno, 'Status' => 380]);
...
//tb1.OH_CLINIC_CODE LIKE '10' and 
	...
	public function Acc420OnCENStatusAsc()
	{
		...
		->where(function($q) use ($date)
		{
			...
			$q->where('AccessionNo.SystemTimeCreated', 'LIKE', $date);
		})
		->whereNotNull('AccessionNo.AccessionMap')
	...
	public function ApprovedRPLeadsHL7()
	{
		...
		foreach($ATrans as $que)
		{
			...
			//checking status from 260 upto 650 
			$status = DB::connection('CMS')->table('AccessionNo')->where('IdQueue', $que->QId)->where('Status','>=','260')->where('Status','<=','900')->where('Type', 'LIKE', 'IMAGING')->get(array('*'));
			if( count($status) != 0 )
			{
				$y = count($status);
			}
	...
	///// For New Status Update
	public function Acc380OnCENStatusAsc()
	{
		$date = date('Y-m-d');
		//$date = "2024-09-25%";
		$accessionData = DB::connection('CMS')->table('AccessionNo')
		->whereIN('AccessionNo.IdBU', ['BAE'] ) // latest branch as of Sep 13 2024
		->where('AccessionNo.Type', 'LIKE', 'LABORATORY')
		->where('AccessionNo.Date', '=', $date)
		//->where('AccessionNo.IdQueue', '=', '11141491')
		->whereNotNull('AccessionNo.AccessionMap')
		->where('AccessionNo.Status', '=', 380)
		->where('AccessionNo.ItemCode', 'NOT LIKE', 'LD005') // drugtest
		//->limit(1)
		->groupBy('AccessionNo.AccessionNo')
		->orderBy('AccessionNo.SystemTimeCreated', 'ASC')
	...
	public function Acc390OnCENStatusAsc()
	{
		$date = date('Y-m-d');
		$accessionData = DB::connection('CMS')->table('AccessionNo')
		->whereIN('AccessionNo.IdBU', ['BAE'] ) // latest branch as of Sep 13 2024
