...
class BAEErosHL7Controller extends Controller
{
    /**
		...
		foreach($qTrans as $que)
		{
			...
			foreach($trans as $trx)
			{
				...
				if(count($AccessionData) == 0)
				{
					...
					DB::connection('CMS')->table('AccessionNo')->insert([ 'IdBU' =>$BUCode ,'AccessionNo' =>$nCode ,'IdQueue' => $que->QId, 'IdCompany' => $trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor ]);
				}
				...
				foreach($insList as $iList)
				{
					...
					if($allowedQty != 0 && ($DepartmentGroup != "CONSULTATION"))
					{
						...
						if(count($haveItem) == 0)
						{
							...
							DB::connection('CMS')->table('AccessionNo')->updateOrInsert(['AccessionNo' => $DDnCode, 'IdQueue' =>$que->QId, 'IdDoctor' =>$trx->IdDoctor, 'IdCompany' =>$trx->IdCompany,'ItemCode' =>$iList->CodeItemPrice, 'IdBU' =>$BUCode ]);
					...
					elseif($CodeItem != 'HPLUS')
					{
						...
						DB::connection('CMS')->table('AccessionNo')->updateOrInsert(['AccessionNo' => $nCode, 'IdQueue' =>$que->QId, 'IdCompany' =>$trx->IdCompany, 'IdDoctor' =>$trx->IdDoctor, 'ItemCode' =>$iList->CodeItemPrice, 'IdBU' =>$BUCode ]);
					}
