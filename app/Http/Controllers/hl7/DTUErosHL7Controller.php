...
class DTUErosHL7Controller extends Controller
{
    /**
	...
	public function AuditRollBack()
	{
		...
		*/
		...
		$auditTransaction =  DB::connection('CMS')->select(" SELECT * FROM `Audit`.`CMSTransactions` tb1  where Id IN ('47075933', '47076008', '47076083')  ");
		...
		*/
		/*
		$auditPaymentHis =  DB::connection('CMS')->select(" SELECT * FROM `Audit`.`CMSPaymentHistory` where `Id` IN( '728783','7125083', '7125158', '7125233')  ");
	...
	public function HCLABUpdateEmail()
	{
		if (DB::connection('oraCENh')->getDatabaseName()) 
		{
			...
			//$CMSEmails = DB::connection('Eros')->table('Patient')->orwhere('InputDate', date('Y-m-d'))->orwhere('UpdateDate', date('Y-m-d'))->orwhere('Email', 'NOT LIKE', '')->whereNull('RDOB')->orWhere('RDOB','LIKE', 'reUpdate')->get(array('*'));
	...
	public function UpdateForPaymentQueue()
	{
		...
		foreach($queDatas as $que)
		{
			echo $que->Code ."<br>";
				...
				{
					DB::connection('CMS')->table('Queue')->where('Id', $que->Id)->update(['Status' => 300]);
				}
	...
	{
		if (DB::connection('oraCENe')->getDatabaseName()) 
		{
			$BUCode = "DTU";
			...
			foreach($queDatas as $que)
			{
				echo $que->Code ."<br>";
					...
					else if( count($selects) == 0 &&  $mins > 15 )
					{
						...
						{
							DB::connection('oraCENe')->table('BILLING_TRX_DTL')->where('BTD_TRXNO', '=', $acc->AccessionNo)->delete();
							DB::connection('oraCENe')->table('BILLING_TRX_HDR')->where('BTH_TRXNO', '=', $acc->AccessionNo)->update(['bth_billing_status' => 'C', 'BTH_CLIN_INFO' => 'Cancelled Transaction' ]);
			...
			echo "Error connection to DB::connection('oraCENe')"; 
		}
	}
	...
	{  
		if (DB::connection('oraCENe')->getDatabaseName()) 
		{
			...
			DB::connection('oraCENe')->beginTransaction();
			...
			foreach($qTrans as $que)
			{
				...
				foreach($trans as $trx)
				{
					...
					{
						$hdrTrx = DB::connection('oraCENe')->table('AUTONO')->where('AUTO_TYPE', 'LIKE', 'TRX')->get(array('auto_number'))[0]->auto_number;
						$NEWhdrTrx = $hdrTrx + 1;
						//echo $nCode = date('y').sprintf('%10d', $NEWhdrTrx); echo "<br>";
						echo $nCode = $BUCode.date('y').str_pad($NEWhdrTrx, 10, '0', STR_PAD_LEFT);
						DB::connection('oraCENe')->table('AUTONO')->where('AUTO_TYPE', 'LIKE', 'TRX')->update(['auto_number' => $NEWhdrTrx ]);
					...
					elseif(count($AccessionData) != 0 )
					{
						...
						DB::connection('oraCENe')->table('BILLING_TRX_DTL')->where('BTD_TRXNO', 'LIKE', $nCode)->delete();
					}
					
					echo $que->FullName; echo "<br>";
					/////// check patient record
					$barangayD = DB::connection('CMS')->table('zip')->where('zip_id', $que->Barangay)->get(array('zip_name'));
					...
					$city = (count($cityD) != 0)?$cityD[0]->city_name:'';
					 DB::connection('oraCENe')->table('PATIENT_MASTER')->updateOrInsert
								(
									...
									,'PM_DOB'				=> $que->DOB
									,'PM_ADDRESS'			=> $que->Address. " ". $barangay
					...
					$insList =  DB::connection('CMS')->table('Transactions')->where('Transactions.IdQueue', $que->QId)->where('Transactions.Status','210')->where('Transactions.IdDoctor',$trx->IdDoctor)->where('Transactions.IdCompany',$trx->IdCompany)
							...
							->get(array('Transactions.Id as TransID','Eros.ItemMaster.OldCode','Transactions.CodeItemPrice','Transactions.AmountItemPrice','Transactions.PriceGroupItemPrice','Transactions.IdItemPrice','ItemMaster.Group as IGroup', 'Company.ErosCode as ComCode', 'Physician.ErosCode as PhyCode'));
					$total = 0;
					...
					foreach($insList as $iList)
					{
						...
						if($iList->PriceGroupItemPrice == "Package" )
						{
							...
							//$selectedASItem = DB::connection('Eros')->table('ItemPrice')->where('Id', $iList->IdItemPrice)->get(array('selected'))[0];
							//if( count($selectedASItem) !=0 && !empty($selectedASItem->selected) && $selectedASItem->selected != "" )
							//{
							//	$BGroup = $selectedASItem->selected;
							//	echo "ItemCode = ".$CodeItem =  $selectedASItem->OldCode;
							//}
							//else
						...
						elseif($iList->PriceGroupItemPrice == "Item"  && count($itemMasterNewCode) != 0)
						{
							...
							echo "ItemCode = ". $CodeItem = ( $itemMasterNewCode[0]->DepartmentGroup == 'LABORATORY'  ) ? $itemMasterNewCode[0]->OldCode : $itemMasterNewCode[0]->Code;
						...
						if($allowedQty != 0 && ($DepartmentGroup != "CONSULTATION"))
						{
							...
							if(count($haveItem) == 0)
							{
								$DDhdrTrx = DB::connection('oraCENe')->table('AUTONO')->where('AUTO_TYPE', 'LIKE', 'TRX')->get(array('auto_number'))[0]->auto_number;
								$DDNEWhdrTrx = $DDhdrTrx + 1;
								//echo $nCode = date('y').sprintf('%10d', $NEWhdrTrx); echo "<br>";
								echo $DDnCode = $BUCode.date('y').str_pad($DDNEWhdrTrx, 10, '0', STR_PAD_LEFT);
								DB::connection('oraCENe')->table('AUTONO')->where('AUTO_TYPE', 'LIKE', 'TRX')->update(['auto_number' => $DDNEWhdrTrx ]);
								...
								echo "<br>";
								DB::connection('oraCENe')->table('BILLING_TRX_DTL')->insert(
									[
										'BTD_TRXNO'			=> $DDnCode
								...
								DB::connection('oraCENe')->table('BILLING_TRX_HDR')->updateOrInsert( 
									['BTH_TRXNO'			=> $DDnCode],
									['BTH_TRXDT'			=> $dateSet
									,'BTH_PID'				=> $que->PCode
									,'BTH_SOURCE'			=> '10'
