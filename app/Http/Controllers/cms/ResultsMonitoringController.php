...
class ResultsMonitoringController extends Controller
{
    /**
	...
	// return route('erosserver.PatientList');
      return view('cms.resultsMonitoringList');
     ...
     public  function getPatientServer($params = array())
   {
		...
		//PM_FULLNAME like '%".$param."%'  AND 
			$xrayImage = DB::connection('CMS')->table('Results')->where('SourceId', oci_result($stid, 'BTH_TRXNO'))->where('SourceCode', 'like', 'XRAY')->where('Action', 'like', 'Upload-XRAY-image.%')->get(array('Id'));
