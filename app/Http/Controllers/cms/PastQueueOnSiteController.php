...
class PastQueueOnSiteController extends Controller
{
	...
	public function getList(Request $request)
	{
		...
		if ($request->ajax()) {
			$queue = PastQueueOnSiteController::pastQueue($searchValue)->get(array('CMS.Queue.Id','CMS.Queue.Code','CMS.Queue.Date as QDate','Eros.Patient.FullName','CMS.Queue.QFullName','CMS.QueueStatus.Name as QueueStatus','CMS.Queue.InputBy','CMS.Queue.Notes'));
    ...
    {	
		...
		return view('cms.pastQueueOnSite');
    }
    ...
    { 
       ...
       // $queue = Queue::todaysQueueID($id)->get();
        //return view('cms.pastQueueOnSite', ['queue' => $queue]);
    }
	...
	$queue = PastQueueOnSiteController::pastQueueID($id)->get(array('Queue.Id','Queue.Code','Queue.AnteDate','Eros.Patient.FullName','CMS.Queue.QFullName','QueueStatus.Name as QueueStatus','Queue.InputBy','Queue.Notes', 'Queue.CancelReason','Queue.IdPatient','Patient.DOB',
	...
	return view('cms.pastQueueOnSiteEdit', ['clinics' => $clinics, 'cancelbt' => $cancelBT, 'defaultClinic' => session('userClinicCode'), 'datas'=>$queue, 'postLink'=>'', 'trans' =>json_encode($trans), 'editor' => $editor, 'AccessRole', $AccessRole, 'disableButton' => $disableButton, 'approveButtonDisabled' => $approveButtonDisabled, 'hl7Btn' => $hl7Btn, 'forPU' => $forPU, 'forEmail' => $forEmail]);    
	      ...
	      // $date = date('Y-m-d', strtotime('-1 day', strtotime(date('Y-m-d')))); 
	       ...
	       return DB::connection('CMS')->table('Queue')
			->where(function($q) use ($pName ) {
					...
					$q->where('Queue.Status', '>=', '210' );
					$q->where('Queue.Status', '<=', '630' );
					$q->whereIn('Queue.IdBU', ['UAT','IMD'] );
					if( !empty($pName))
					{
