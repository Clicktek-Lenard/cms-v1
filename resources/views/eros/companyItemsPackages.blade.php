		...
		<div class="col-menu-10">
			<div class="header-crumb">
				<ol class="breadcrumb" style=" border-radius:0; " >
					...
					<li class="active"><a href="#">Item and Packages <span class="badge" style="top:-9px; position:relative;"></span></a></li>
				</ol>
			</div>
		</div>
	</div>
		...
		<div class="col-menu">
			<div class="btn-group col-xs-12 col-sm-12 col-md-12" style="background-color:#8F8F8F; height:inherit;">
				<div class="col-xs-12 col-sm-10 col-sm-offset-2 col-md-8 col-md-offset-4 col-lg-6 col-lg-offset-6">
					<a @if( strpos(session('userRole') , '"ldap_role":"['.strtoupper(\Request::segment(2)).']"') !== false ) href="{{ url(session('userBU').'/erosui/company/itemspackages/lab2lab/'.$datas[0]->Id.'/') }}"  @else disabled="disabled"  @endif class="btn btn-danger col-xs-4 col-sm-4 col-md-4 col-lg-4" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Upload Price</a>
					<a @if( strpos(session('userRole') , '"ldap_role":"['.strtoupper(\Request::segment(2)).']"') !== false ) href="{{ url(session('userBU').'/erosui/company/itemspackages/package/'.$datas[0]->Id.'/') }}" @else disabled="disabled"  @endif class="btn btn-warning col-xs-4 col-sm-4 col-md-4 col-lg-4" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Add Package</a>
					<a @if( strpos(session('userRole') , '"ldap_role":"['.strtoupper(\Request::segment(2)).']"') !== false ) href="{{ url(session('userBU').'/erosui/company/itemspackages/item/'.$datas[0]->Id.'/') }}" @else disabled="disabled" @endif  class="btn btn-primary col-xs-4 col-sm-4 col-md-4 col-lg-4" style="border-radius:0px; line-height:29px;" role="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Add Item</a>					
				</div>
			</div>
		</div>
	</div>
...
$(document).ready(function(e) {
	...
	var table = $('#QueueListTable').DataTable({
		...
		autoWidth		: false,
		deferRender		: true,
		createdRow		: function ( row, data, index ) { $(row).attr('data-toggle-queueId', data.Id).attr('data-toggle-priceGroup', data.PriceGroup); },
		columns			: [
		{ "data": null },
		...
		{ "data": "PriceGroup", "render": function(data,type,row,meta) { return '<div class="PriceGroup wrap-row">'+data+'</div>'; } },
		{ "data": "ClinicCode", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } },
	...
	$('#QueueListTable').on('click','.data-row',function(e){
		...
		var PriceGroup = $(this).closest('tr').data('toggle-pricegroup'); 
		var hyperlink = document.createElement('a');
		
		hyperlink.href = PriceGroup.toLowerCase()+'/'+{{ $datas[0]->Id }}+'/edit/?id='+id;
