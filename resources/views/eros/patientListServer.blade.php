		...
		<div class="navbar-fixed-top crumb" >
			...
			<div class="header-crumb">
				<ol class="breadcrumb" style=" border-radius:0; " >
					<li class="active"><a href="{{ url(session('userBUCode').'/erosPatientServer') }}" class="waiting">EROS - Today's Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
				</ol>
			</div>
			</div>
		</div>
...
$(document).ready(function(e)
{
	...
	var table = $('#QueueListTable').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: "{{ route('erosserver.PatientList') }}",
		autoWidth		: false,
		deferRender		: true,
	...
	$('#QueueListTable').on('click','.data-row',function(e){
		...
		hyperlink.href = 'erosPatientServer/'+id+'/edit';
