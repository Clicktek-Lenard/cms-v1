        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ url(session('userBUCode').'/enrollment/cardreceiving') }}" class="waiting">Card <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
	...
	<div class="body-content row">
		<div class="col-menu-15">
				<div class="panel panel-success">
					<div class="panel-heading" style="line-height:12px;">Receiving</div>
		...
		<div class="col-menu-15">
				<div class="panel panel-danger">
					<div class="panel-heading" style="line-height:12px;">Received</div>
						<div class="panel-body">
							<div class="row">
								<div class="table-queues">
								</div>
							</div>
						</div>
				  </div>      
...
{	
	...
	$html += "</tr>";
		...
		var table = $('#QueueListTable').DataTable({
			...
			order			: [ 3, 'desc' ],
			dom:            "frtiS",
			scrollY: $(document).height()-$('.navbar-fixed-top.crumb').height()-$('.navbar-fixed-bottom').height()-460,
		});
		...
		$('.dataTables_filter input').on('input', function() {
		 ...
		 function openModalWithBarcode(barcode) {
			...
			addModal.setTitle("Receiver - View");
			addModal.setData("pageToLoad", "{{ '/enrollment/pages/enrollmentReceived/'}}" + rowId + '/edit');
...
$(document).ready(function(e)
{
	$html = "<div class=\"table-responsive\"><table id=\"QueueListTables\" class=\"table table-striped table-hover dt-responsive display \" style=\"width:100%;\" cellspacing=\"0\" >";
	$html += "<thead>";
          $html += "<tr>";
	$html += "<th></th>";
	$html += "<th>Card Number</th>";
	$html += "<th>Date Registered</th>";
	$html += "<th>Registered To</th>";
	$html += "<th>Registered By</th>";
	...
	$html += "</tr>";
		...
		var table = $('#QueueListTables').DataTable({
			data			: data,
			autoWidth		: false,
			deferRender		: true,
			createdRow		: function ( row, data, index ) { $(row).attr('data-toggle-queueId', data.Id); },
			...
			{ "data": "ReleaseTo", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } },
			{ "data": "ReleaseBy", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } },
			{ "data": "ReceivedDate", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } },
			{ "data": "ReceivedBy", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } }],
			responsive		: { details: { type: 'column' } },
			columnDefs		: [
				{className: 'control', orderable: false, targets: 0, "width":"15px",defaultContent: ""},
