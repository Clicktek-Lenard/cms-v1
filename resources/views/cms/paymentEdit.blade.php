...
<div class="container-fluid">
	...
	<div class="btn-group col-xs-12 col-sm-12 col-md-12" style="background-color:#8F8F8F; height:inherit;">
		<div class="col-xs-12 col-sm-10 col-sm-offset-2 col-md-8 col-md-offset-4 col-lg-6 col-lg-offset-6">
			<a @if($datas->Status <= 201 && $datas->Status == 900) disabled="disabled" @endif class="drfbtn btn btn-success col-xs-2 col-sm-2 col-md-2 col-lg-2" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> DRF </a>
			...
			<a @if($receiptButton) disabled="disabled" @endif @if( $CS == 0 )  disabled="disabled"  @endif    class="chargebtn btn btn-danger col-xs-2 col-sm-2 col-md-2 col-lg-2" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Charge Slip</a>
			<a @if($receiptButton) disabled="disabled" @endif @if( $OR == 0 )  disabled="disabled"  @endif    class="orbtn btn btn-warning col-xs-2 col-sm-2 col-md-2 col-lg-2" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> O.R.</a>
...
$(document).ready(function(e) {
	...
	$('.drfbtn').on('click',function(e){  
		if($(this).attr('disabled') !== 'disabled' )
		{
			var id = '{{ $datas->Id }}';
			var userId = '{{ Auth::user()->id }}';
			var hyperlink = document.createElement('a');
			hyperlink.href = "http://{{$_SERVER['SERVER_ADDR']}}:8080/jasperserver/flow.html?_flowId=viewReportFlow&standAlone=true&_flowId=viewReportFlow&ParentFolderUri=%2Freports%2FBilling&reportUnit=%2Freports%2FBilling%2FDRF&j_username=report&j_password=DnCMSReport&output=pdf&appointmentID="+id+"&myID="+userId;
			var mouseEvent = new MouseEvent('click', {
				view: window,
				bubbles: true,
				cancelable: true
	...
	$('.rtngbtn').on('click',function(e){  
		if($(this).attr('disabled') !== 'disabled' )
		{
			...
			var userId = '{{ Auth::user()->id }}';
			var hyperlink = document.createElement('a');
			hyperlink.href = "http://{{$_SERVER['SERVER_ADDR']}}:8080/jasperserver/flow.html?_flowId=viewReportFlow&standAlone=true&_flowId=viewReportFlow&ParentFolderUri=%2Freports%2FBilling&reportUnit=%2Freports%2FBilling%2FRS_v3&j_username=report&j_password=DnCMSReport&output=pdf&appointmentID="+id+"&myID="+userId;
			var mouseEvent = new MouseEvent('click', {
				view: window,
				bubbles: true,
				cancelable: true
	...
	$('.orbtn').on('click',function(e){
		if($(this).attr('disabled') !== 'disabled' )
		{
			...
			var hyperlink = document.createElement('a');
			hyperlink.href = "http://{{$_SERVER['SERVER_ADDR']}}:8080/jasperserver/flow.html?_flowId=viewReportFlow&standAlone=true&_flowId=viewReportFlow&ParentFolderUri=%2Freports%2FBilling&reportUnit=%2Freports%2FBilling%2FOR_v2.5&j_username=report&j_password=DnCMSReport&output=pdf&appointmentID="+id+"&myID="+userId;
