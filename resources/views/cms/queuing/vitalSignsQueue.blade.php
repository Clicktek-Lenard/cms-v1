        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ url(session('userBUCode').'/kiosk/vitalsignsqueue') }}" class="waiting"> Vital Signs Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.call-button', function () {
        ...
        $.ajax({
            ...
            data: { actionBy : authenticatedUsername, room : 'Vital Signs', action : 'CALL', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
            success: function (response) {
    ...
    $(document).on('click', '.in-button', function () {
        ...
        setTimeout(function() {
            ...
            $.when.apply($, deferreds).done(function () {
                window.location.href = '/doctor/vitals/' + idQueueCMS +'/edit';
            }).fail(function (error) {
    ...
    $(document).on('click', '.pause-button-circle', function () {
        ...
        if (icon.hasClass('fa-pause')) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Vital Signs', action : 'HOLD QUEUE', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Vital Signs', action : 'RESUME QUEUE', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
