        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li><a href="{{ '/kiosk/consultationqueue' }}">Consultation Queue <span class="badge cms-font" style="top:-9px; position:relative;"></span></a></li>
        ...
        <div class="col-menu">
            <div class="btn-group col-xs-12 col-sm-12 col-md-12" style="background-color:#8F8F8F; height:inherit;">
				<div class="col-xs-12 col-sm-10 col-sm-offset-2 col-md-8 col-md-offset-6 col-lg-6 col-lg-offset-8">
                    <a href="" class="btn btn-success col-xs-4 col-sm-4 col-md-4 col-lg-4" style="visibility: hidden; border-radius:0px; line-height:29px;" role="button">
...
// 		var pageToLoad = dialog.getData('pageToLoad')+"/"+dialog.getData('Code')+"/Clinic";
	...
	var table = $('#TransactionListTable').DataTable({
		...
		createdRow		: function ( row, data, index ) {
			...
			if (packageItems.hasOwnProperty(packageCode)) {
				...
				// Access the array using the property key
				const items = packageData[packageCode];

				// Check if the items are an array
				if (Array.isArray(items)) {
					let packageContentsCode = '<br><ul style="padding-left: 0px">';
					let packageContentsDesc = '<br><ul style="padding-left: 0px">';
					items.forEach(function(item) {
						// Only add checkbox if DepartmentGroup is 'LABORATORY'
						if (item.DepartmentGroup === 'CLINIC' || item.DepartmentGroup === 'CONSULTATION') {
							packageContentsCode += `
							...
							// noteContents += `<textarea name="notes[]" class="form-control" rows="1" placeholder="Enter notes for ${item.Code}" style="font-weight: normal;margin-bottom: 2px;"></textarea>`;
						}
					});
					packageContentsCode += '</ul>';
					packageContentsDesc += '</ul>';

					// Instead of overwriting, append the new content to the existing cell content
					const existingContent = $(row).find('td:eq(3)').html(); // Get existing content
					$(row).find('td:eq(3)').html(existingContent + packageContentsCode); // Append new content

					const existingContent2 = $(row).find('td:eq(5)').html(); // Get existing content
					$(row).find('td:eq(5)').html(existingContent2 + packageContentsDesc); // Append new content

				} else {
					console.warn(`Expected an array for packageItems[${packageCode}], but got:`, items);
				}
			} else {
				console.warn(`Package code ${packageCode} does not exist in packageItems.`);
			}
		},
		columns			: [
		{ "data": null },
...
$('.exitbtn').on('click',function(e){
	...
	$.ajax({
		...
		success: function (response) {
			...
			window.location.href = '/kiosk/consultationqueue';
		},
