        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ url(session('userBUCode').'/kiosk/releasingqueue') }}" class="waiting"> Releasing Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
...
$(document).ready(function (e) {
    ...
    var table = $('#QueueListTable').DataTable({
        ...
        autoWidth: false,
        deferRender: true,
        createdRow: function (row, data, index) { $(row).attr('data-toggle-queueId', data.Id).attr('data-toggle-idPatient', data.IdPatient); },
        columns: [
            { "data": null },
            { 
                "data": "QRCode",
                "render": function(data, type, row, meta) {
                    ...
                    var branch = row.IdBU;
                    var priority = row.Priority === 1 ? 'RELEASING' : (row.Priority === 0 ? 'RELEASING' : (row.Priority === 2 ? 'RELEASING' : (row.Priority === 3 ? 'RELEASING' : '')));
                    var queueNo = branch + '-' + priority +'-' + data; // Combine branch and QRCode

                    if(currentRoom === 'ON HOLD'){
                        return '<div class="rectangle text-center on-hold">' + queueNo + '</div>';
                    } else {
                        return '<div class="rectangle text-center">' + queueNo + '</div>';
                    }
                }
            },            
            { 
            ...
            },
            {
                ...
                "render": function(data, type, row, meta) {
                    ...
                    if (data !== null && data !== undefined && data !== "") {
                        ...
                        // Return the buttons HTML with an absolutely positioned badge-notify
                        if(currentRoom === 'ON HOLD'){
                        ...
                        // Return the buttons HTML with an empty notification badge
                        return '<div class="wrap-row text-center" style="position: relative;">' + buttonsHtml +
        ...
        columnDefs: [
            ...
            { targets: 1, "width":"100px", autoWidth: false, orderable: false },
            { targets: 2, "width":"30px", autoWidth: false, orderable: false  },
            { targets: 3, "width":"100px", autoWidth: false, orderable: false  },
            { targets: 4, "width":"50px", autoWidth: false, orderable: false  },
    ...
    var priorityFilter = $('<select id="priorityFilter" class="form-control ml-2" disabled><option value="" selected>ALL</option><option value="0">REGULAR</option><option value="1">PRIORITY</option></select>')
...
$(document).ready(function () {
    ...
    $(document).on('click', '.call-button', function () {
        ...
        $.ajax({
            ...
            data: { actionBy : authenticatedUsername, room : 'Releasing', action : 'CALL', queueno : queueNo.substring(14), idpatient : IdPatient, kioskid : queueID},
            success: function (response) {
    ...
    $(document).on('click', '.in-button', function () {
        ...
        $.ajax({
            ...
            data: { actionBy : authenticatedUsername, room : 'Releasing', action : 'IN', queueno : queueNo.substring(14), idpatient : IdPatient, erosidpatient : erosid, kioskid : queueID},
            success: function (response) {
        ...
        // window.location.href = 'queue/create?kioskPID=' + queueID;  
    });
    ...
    $(document).on('click', '.pause-button-circle', function () {
        ...
        if (icon.hasClass('fa-pause')) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Releasing', action : 'HOLD QUEUE', queueno : queueNo.substring(14), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Releasing', action : 'RESUME QUEUE', queueno : queueNo.substring(14), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
