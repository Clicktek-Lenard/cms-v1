        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ url(session('userBUCode').'/kiosk/receptionqueue') }}" class="waiting"> Reception Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
        ...
        columns: [
            ...
            { 
                ...
                "render": function(data, type, row, meta) {
                    ...
                    var priority = row.Priority === 1 ? 'P' : (row.Priority === 0 ? 'R' : (row.Priority === 2 ? 'R' : (row.Priority === 3 ? 'P' : '')));
...
$(document).ready(function () {
    ...
    $(document).on('click', '.call-button', function () {
        ...
        $.ajax({
            ...
            data: { actionBy : authenticatedUsername, room : 'Reception', action : 'CALL', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
            success: function (response) {
    ...
    socket.on('currentRoom', function (data) {
        ...
        if (currentRoom === 'ON HOLD') {
            ...
            badge.hide();
        } else if (currentRoom === 'Resume Queue'){
    ...
    $(document).on('click', '.pause-button-circle', function () {
        ...
        if (icon.hasClass('fa-pause')) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Reception', action : 'HOLD QUEUE', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
            ...
            $.ajax({
                ...
                data: { actionBy : authenticatedUsername, room : 'Reception', action : 'RESUME QUEUE', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
                success: function (response) {
