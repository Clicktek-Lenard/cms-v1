        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ url(session('userBUCode').'/kiosk/extractionqueue') }}" class="waiting"> Extraction Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
...
$(document).ready(function (e) {
    $html = "<div class=\"table-responsive\"><table id=\"QueueListTable\" data-queue-type=\"EXTRACTION\" class=\"table table-striped table-hover dt-responsive display \" style=\"width:100%;\" cellspacing=\"0\" >";
    $html += "<thead>";
    $html += "<tr>";
    $html += "<th></th>";
    $html += "<th>Name</th>";
    ...
    var table = $('#QueueListTable').DataTable({
        ajax: {
            url: '{{ route('getqueuedatapaid') }}',
            data: function(d) {
                d.station = @json($station);
            },
        ...
        columns: [
            ...
            { 
                ...
                "render": function(data, type, row, meta) {
                    ...
                    if (currentOnHold.includes('EXTRACTION')) {
            ...
            },    
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    // Allowed stations
                    const allowedStations = ["LABORATORY", "DRUG TEST"];

                    // Split, filter, and wrap each valid station in a badge
                    const badges = data.split(', ')
                        .filter(station => allowedStations.includes(station.trim()))
                        .map(station => `<div class="badge badge-procedures">${station}</div>`)
                        .join('');

                    // Wrap in a flex column div for centering
                    return badges ? `<div style="display: flex; flex-direction: column; align-items: center;">${badges}</div>` : '';
                }
            },
            { 
                "data": "Priority",  
            ...
            },
            {
                ...
                "render": function(data, type, row, meta) {
                    ...
                    var isOnHold = row.OnHold && row.OnHold.includes("EXTRACTION"); // Check if OnHold contains "LABORATORY"
        ...
        columnDefs: [
            ...
            { targets: 7, "width":"50px", autoWidth: false, orderable: false,  },
            {
                targets: 8, 
                visible: false, 
                orderable: true, 
                render: function(data, type, row, meta) {
                    var data = (row.OnHold && row.OnHold.includes('EXTRACTION')) ? '1' : '0'; 
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    var code = row.Code;
                    return '<div class="wrap-row text-center hidden">' + code + '</div>';
                }
            }
        ],
        order: [[8, 'asc'],[2, 'asc']],
    ...
    @if(strpos(session('userRole'), '"ldap_role":"[EXTRACTION-DT]"') !== false)
            table.column(3).search("DRUG TEST").draw();
    @else
            table.column(3).search("LABORATORY").draw();
    @endif
...
var department = "{{ strtoupper($counter->Department) }}";
@if(strpos(session('userRole'), '"ldap_role":"[EXTRACTION-DT]"') !== false)
    department = "DRUG TEST"; 
@endif


console.log("Department:", department); // Debugging: check the value
$(document).ready(function () {
    ...
    $(document).on('click', '.call-button', function () {
        ...
        $.ajax({
            ...
            data: { actionBy : authenticatedUsername, room : 'Laboratory', action : 'CALL', queueno : queueNo.substring(6), idpatient : IdPatient, kioskid : queueID},
            success: function (response) {
    ...
    socket.on('currentRoom', function (data) {
        ...
        console.log(currentQueueType, department);
    ...
    $(document).on('click', '.in-button', function () {
        ...
        setTimeout(function() {
            ...
            $.when.apply($, deferreds).done(function () {
                window.location.href = 'extractionqueue/' + idQueueCMS +'/edit?department=' + encodeURIComponent(department);
            }).fail(function (error) {
    ...
    $(document).on('click', '.pause-button-circle', function () {
        ...
        if (isHolding) {
            ...
            $.ajax({
                ...
                headers: { 'X-CSRF-TOKEN': csrfToken },
                data: { queueID: queueID, roomName: 'EXTRACTION', action: 'hold' },
                success: function (response) {
                    console.log('Status updated successfully');
                    socket.emit('currentRoom', { queueID: queueID, department: 'EXTRACTION', onHold: 'EXTRACTION', action: 'HOLD' });
                },
                error: function (error) {
                    console.error('Error updating status:', error);
                }
            ...
            $.ajax({
                ...
                url: '{{ route('actionlog') }}',
                headers: { 'X-CSRF-TOKEN': csrfToken },
                data: { actionBy: authenticatedUsername, room: 'Extraction', action: 'HOLD QUEUE', queueno: queueNo.substring(6), idpatient: IdPatient, kioskid: queueID },
                error: function (error) {
                    console.error('Error updating status:', error);
                }
            });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (!onHold.includes('EXTRACTION')) {
                        ...
                        socket.emit('currentRoom', { queueID: queueID, department: 'EXTRACTION', onHold: onHold, action: 'RESUME' });
                    }
                },
                error: function (error) {
                    console.error('Error updating status:', error);
            ...
            $.ajax({
                ...
                url: '{{ route('actionlog') }}',
                headers: { 'X-CSRF-TOKEN': csrfToken },
                data: { actionBy: authenticatedUsername, room: 'Extraction', action: 'RESUME QUEUE', queueno: queueNo.substring(6), idpatient: IdPatient, kioskid: queueID },
                error: function (error) {
                    console.error('Error updating status:', error);
                }
            });
