        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li><a href="{{ '/kiosk/imagingqueue' }}">Imaging Queue <span class="badge cms-font" style="top:-9px; position:relative;"></span></a></li>
...
var packageModal = new BootstrapDialog({
	message: function(dialog) {
		...
		var pageToLoad = dialog.getData('pageToLoad')+"/"+dialog.getData('Code')+"/Imaging";
			...
			if (packageItems.hasOwnProperty(packageCode)) {
				...
				if (Array.isArray(items)) {
					...
					items.forEach(function (item) {
						...
						let receivedPackage = receivingStatus.find(packageItem => 
							packageItem.ItemCode === item.Code && 
							packageItem.IdQueue == IdQueue && 
							packageItem.IdTransaction === data.Id
						);

						// Only process items if their DepartmentGroup is in the stations list
						if (stations.includes(item.DepartmentGroup)) {
							if (receivedPackage && receivedPackage.Status === "Received" && stations.includes(item.DepartmentGroup)) {
								packageContentsCode += `
								...
								noteContents += `
									<textarea name="notes[]" class="form-control" rows="1" 
										placeholder="Notes for ${item.Code}" 
										style="font-weight: normal;margin-bottom: 2px;" 
										disabled>${receivedPackage.Notes || ''}</textarea>`;
							} else { 
								packageContentsCode += `
									<li style="margin-bottom: 5px;">
								...
								noteContents += `
									<textarea name="notes[]" class="form-control" rows="1" 
										placeholder="Enter notes for ${item.Code}" 
										style="font-weight: normal;margin-bottom: 2px;"></textarea>`;
...
const station = "{{ is_array($stations) ? $stations[0] : $stations }}";
...
$('.exitbtn').on('click',function(e){
	...
	$.ajax({
		...
		headers: { 'X-CSRF-TOKEN': csrfToken },
		data: { queueID: queueID, station: station },
		success: function (response) {
			console.log('Exit room successfully');
			window.location.href = '/kiosk/imagingqueue';
		},
		error: function (error) {
			console.error('Error updating status:', error);
		}
