        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li><a href="{{ '/kiosk/extractionqueue' }}">Extraction Queue <span class="badge cms-font" style="top:-9px; position:relative;"></span></a></li>
    ...
    <div class="navbar-fixed-bottom">
		<div class="col-menu">
            <div class="btn-group col-xs-12 col-sm-12 col-md-12" style="background-color:#8F8F8F; height:inherit;">
				<div class="col-xs-12 col-sm-10 col-sm-offset-2 col-md-8 col-md-offset-6 col-lg-6 col-lg-offset-8">
					<a class="receivebtn btn btn-warning col-xs-4 col-sm-4 col-md-4 col-lg-4" style="  border-radius:0px; line-height:29px;" type="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Receive</a>
					<a class="exitbtn saving btn btn-primary col-xs-4 col-sm-4 col-md-4 col-lg-4" style="border-radius:0px; line-height:29px;" role="button"><span class="spinnerbtn"><i class="fa fa-spinner fa-spin hide"></i></span> Exit Room</a>
                </div>
            </div>
        </div>
    </div>
...
// console.log($('input[name="IdPatient"]').val());
...
// 		var pageToLoad = dialog.getData('pageToLoad')+"/"+dialog.getData('Code')+"/Laboratory";
...
var editTransactionModal = new BootstrapDialog({
	...
	type: BootstrapDialog.TYPE_SUCCESS,
	data: {
		'pageToLoad': "{{ '/cms/pages/editTransaction' }}"
	},
	animate: false,
	closable: false,
	buttons: [{
...
var anteDateModal = new BootstrapDialog({
	message: function(dialog) {
...

	{
		...
		action: function (modalRef){
			...
			if( parent.required($('#anteDateModal')) ) return false;
	
			var form = $('#anteDateModal');
...
$(document).ready(function(e) {
	var Canceltransaction = "{{$canceltransaction}}";
	...
	var receivingStatus = {!! $receivingStatus !!};
	...
	var table = $('#TransactionListTable').DataTable({
		...
		createdRow		: function ( row, data, index ) {
			...
			const packageCode = data.CodeItemPrice; 

			console.log("Checking package code:", packageCode); // Debugging line

			// Find if the row's ItemCode exists in receivingStatus
			const receivedItem = receivingStatus.find(item => item.ItemCode === data.CodeItemPrice && item.IdTransaction === data.Id);

			if (receivedItem && receivedItem.Status === "Received") {
				// Remove the checkbox
				$(row).find('td:eq(1)').html(''); 
				
				// Set the Notes field with the received note and disable it
				$(row).find('td:eq(6) textarea').val(receivedItem.Notes).prop('disabled', true);
			}

			// Check if the packageCode exists in packageItems
			if (packageItems.hasOwnProperty(packageCode)) {
				const packageData = packageItems[packageCode];
				const items = packageData[packageCode];

				if (Array.isArray(items)) {
					let packageContentsCode = '<br><ul style="padding-left: 0px">';
					let packageContentsDesc = '<br><ul style="padding-left: 0px">';
					let noteContents = '<br><br>';

					let allReceived = true;  // Flag to track if all items are received

					items.forEach(function(item) {
						const IdQueue = $('input[name="_queueid"]').val();

						let receivedPackage = receivingStatus.find(packageItem => packageItem.ItemCode === item.Code && packageItem.IdQueue == IdQueue && packageItem.IdTransaction === data.Id);

						if (receivedPackage && receivedPackage.Status === "Received" && item.DepartmentGroup === 'LABORATORY') {
							packageContentsCode += 
							...
							noteContents += 
								`<textarea name="notes[]" class="form-control" rows="1" placeholder="Notes for ${item.Code}" style="font-weight: normal;margin-bottom: 2px;" disabled>${receivedPackage.Notes || ''}</textarea>`;
						} else if (item.DepartmentGroup === 'LABORATORY') {
							packageContentsCode += 
								`<li style="margin-bottom: 5px;">
									<input type="checkbox" class="package-item-checkbox" data-code="${item.Code}">
									${item.Code}
								</li>`;
							...
							noteContents += 
								`<textarea name="notes[]" class="form-control" rows="1" placeholder="Enter notes for ${item.Code}" style="font-weight: normal;margin-bottom: 2px;"></textarea>`;

							allReceived = false; // Mark as false if any item is not received
						}
					});

					packageContentsCode += '</ul>';
					packageContentsDesc += '</ul>';

					// Append new content to the existing cell content
					const existingContent = $(row).find('td:eq(3)').html();
					$(row).find('td:eq(3)').html(existingContent + packageContentsCode);

					const existingContent2 = $(row).find('td:eq(5)').html();
					$(row).find('td:eq(5)').html(existingContent2 + packageContentsDesc);

					$(row).find('td:eq(6)').html(noteContents);

					// Remove main package checkbox if all items are received
					if (allReceived) {
						$(row).find('td:eq(1)').html('');  // Remove the checkbox
					}
				} else {
					console.warn(`Expected an array for packageItems[${packageCode}], but got:`, items);
				}
			}
		},
		columns			: [
		{ "data": null },
		{ "data": "Id", "orderDataType": "dom-checkbox" ,"render": function(data,type,row,meta) {  return '<input type="checkbox" name="id[]" data-toggle-QueueStatus="'+row.QueueStatus+'" value="'+data+'">';}  },
		...
		{ "data": "QueueStatus", "render": function(data,type,row,meta) 
				//1-5-24 IF Status === NULL CHANGE TEXT TO 'Click Save'//
		...
		columnDefs		: [
			...
			{ data: null,  targets: 2, "width": "150px" },
			{ data: null, targets: 3, "width":"90px" },
			{ "data": "NameDoctor", targets: 4, "width":"300px" },
			{ "data": "DescriptionItemPrice",targets: 5, "width":"300px", defaultContent: "" },
			{ "data": "",targets: 6, "width":"200px"},
			{ "data": "TransactionType",targets: 7, "width":"80px"},
			{ "data": "QueueStatus",targets: 8, "width":"80px"},
	...
	$('#TransactionListTable tbody').on('click', 'input[type="checkbox"]', function() {
		var $checkbox = $(this);
		var $row = $checkbox.closest('tr');
		var data = table.row($row).data();

		var isMainCheckbox = $checkbox.attr('name') === 'id[]';  // Main checkbox for package
		var isPackageItemCheckbox = $checkbox.hasClass('package-item-checkbox');  // Checkbox for package item

		if (isMainCheckbox) {
			var packageCode = data.CodeItemPrice;
			var $packageCheckboxes = $row.find('input.package-item-checkbox');

			if ($checkbox.is(':checked')) {
				$packageCheckboxes.prop('checked', true);
				$checkbox.prop('indeterminate', false);
				$row.addClass('selected');  // Highlight row when fully checked
			} else {
				$packageCheckboxes.prop('checked', false);
				$checkbox.prop('indeterminate', false);
				$row.removeClass('selected');  // Remove highlight when unchecked
			}
		} else if (isPackageItemCheckbox) {
			var $packageCheckboxes = $row.find('input.package-item-checkbox');
			var $mainPackageCheckbox = $row.find('input[name="id[]"]');

			var checkedCount = $packageCheckboxes.filter(':checked').length;
			var totalCount = $packageCheckboxes.length;

			if (checkedCount === totalCount) {
				// All items are checked: check main checkbox and highlight row
				$mainPackageCheckbox.prop('checked', true).prop('indeterminate', false);
				$row.addClass('selected');
			} else if (checkedCount > 0) {
				// Some items are checked: set main checkbox to indeterminate, but don't highlight row
				$mainPackageCheckbox.prop('checked', false).prop('indeterminate', true);
				$row.removeClass('selected');
			} else {
				// No items are checked: uncheck main checkbox and remove highlight
				$mainPackageCheckbox.prop('checked', false).prop('indeterminate', false);
				$row.removeClass('selected');
			}
		} else {
			// Regular row checkbox (non-package) handling
			if ($checkbox.is(':checked')) {
				$row.addClass('selected');
			} else {
				$row.removeClass('selected');
	...
	$('.anteDate').datepicker({ maxDate: '+0', firstDay: 1, dateFormat: 'mm/dd/yy', changeMonth: true, changeYear: true, yearRange: 'c-100:c+10'});
	...
	$('.receivebtn').on('click', function(e){
		...
		var IdQueue = $('input[name="_queueid"]').val();
		...
		table.rows().every(function() {
			var data = this.data();
			var $row = $(this.node());
			var $mainCheckbox = $row.find('input[name="id[]"]');
			var $packageCheckboxes = $row.find('input.package-item-checkbox:checked');

			// If the main checkbox is checked, avoid double counting sub-items
			if ($mainCheckbox.is(':checked') && data.PriceGroupItemPrice === "Package") {
				if (!$packageCheckboxes.length) {
					// No sub-items checked, add all package items
					$row.find('input.package-item-checkbox').each(function() {
						var packageItemCode = $(this).data('code');
						selectedItems.push({
							id: $mainCheckbox.val(),
							code: packageItemCode,
							packageCode: data.CodeItemPrice,
							notes: $row.find(`textarea[placeholder="Enter notes for ${packageItemCode}"]`).val() || ''
						});
					});
				}
			} 

			// Process individual package sub-items if they are checked separately
			if ($packageCheckboxes.length) {
			...
			// Process single items that are not part of a package
			if ($mainCheckbox.is(':checked') && data.PriceGroupItemPrice === "Item") {
				selectedItems.push({
					id: $mainCheckbox.val(),
					code: data.CodeItemPrice,
		...
		$.ajax({
			type: 'POST',
			url: '{{ route('receivespecimen') }}',
			headers: { 'X-CSRF-TOKEN': csrfToken },
			data: { selectedItems: selectedItems, idQueue: IdQueue },
			success: function (response) {
				// console.log('Received specimen successfully');
				window.location.reload();
			},
...
var department = "{{ strpos(session('userRole'), '"ldap_role":"[EXTRACTION-DT]"') !== false ? 'DRUG TEST' : 'LABORATORY' }}";
...
$('.exitbtn').on('click',function(e){
	...
	$.ajax({
		...
		headers: { 'X-CSRF-TOKEN': csrfToken },
		data: { queueID: queueID, station: department },
		success: function (response) {
			console.log('Exit room successfully');
			window.location.href = '/kiosk/extractionqueue';
		},
		error: function (error) {
			console.error('Error updating status:', error);
		}
