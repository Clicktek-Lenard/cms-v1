        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="#">Completed Queue <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
...
$(document).ready(function(e)
{
	...
	$html += "</tr>";
		...
		var table = $('#QueueListTable').DataTable({
			...
			autoWidth		: false,
			deferRender		: true,
			createdRow		: function ( row, data, index ) { $(row).attr('data-toggle-queueId', data.Id).attr('data-toggle-AItemCode', data.AItemCode); },
			columns			: [
			{ "data": null },
			{ "data": "QCode", "render": function(data,type,row,meta) { return '<div class="wrap-row">'+data+'</div>'; } },
		...
		$('#QueueListTable').on('click','.data-row',function(e){
			...
			if( $(this).closest('tr').data('toggle-aitemcode') == 'CI002' )
			{
				hyperlink.href = '/doctor/pe/'+id+'/edit';
			}
			else
			{
				hyperlink.href = 'historyqueue/'+id+'/edit';
			}
