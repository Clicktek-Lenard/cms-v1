		...
		<div class="navbar-fixed-top crumb" >
			...
			<div class="header-crumb">
				<ol class="breadcrumb" style=" border-radius:0; " >
					<li class="active"><a href="{{ url(session('userBUCode').'/processing/evaluated') }}" class="no-waiting">Company Evaluated <span class="badge" style="top:-9px; position:relative;"></span></a></li>
				</ol>
			</div>
			</div>
		</div>
...
$(document).ready(function(e)
{
	...
	var table = $('#QueueListTable').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: "{{ route('cmsEvaluatedCompany.CompanyList') }}" + '/?sid={{ $sid }}' ,
		autoWidth		: false,
		deferRender		: true,
	...
	$('#QueueListTable').on('click','.data-row',function(e){
		...
		hyperlink.href = 'evaluated/company/'+id+'/edit';
