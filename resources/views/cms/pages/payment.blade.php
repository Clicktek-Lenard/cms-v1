...
{ 
	...
	$('tbody td.sgroup input[type="checkbox"]:checked', $table).each(function(){ 
...
function checkDoctor(checkbox) {
    var doctorName = checkbox.value;
    var table = $('#QueueListTable').DataTable();
    var rows = $('#QueueListTable tbody tr').toArray(); // Get all rows using jQuery

    rows.forEach(function(row) {
        var rowData = table.row($(row)).data(); // Use jQuery to wrap row for DataTable
        if (rowData && rowData.NameDoctor === doctorName) { // Check if rowData exists
...
function checkGroupCheckboxStatus(table) {
    var groups = $('.group-checkbox');
    var anyRowSelected = false;

    groups.each(function() {
        var groupName = $(this).val();
        var groupCheckbox = $(this);
        var groupRows = $('#QueueListTable tbody tr').filter(function() {
            var rowData = table.row($(this)).data();
            return rowData && rowData.NameDoctor === groupName && (rowData.Status === 201 || rowData.Status === 205);
        });

        var selectedRows = groupRows.filter('.selected');

        var allRowsSelected = selectedRows.length === groupRows.length;
        var someRowsSelected = selectedRows.length > 0 && !allRowsSelected;
        ...
        // Disable group-checkbox and its related rows' checkboxes
        if (allRowsSelected || someRowsSelected) {
            groupCheckbox.prop('disabled', false);
            groupRows.find('input[type="checkbox"]').prop('disabled', false);
        } else {
            groupCheckbox.prop('disabled', true);
            groupRows.find('input[type="checkbox"]').prop('disabled', true);
        }
        
        // Enable checkboxes for rows with the same company if any row is selected within the group
        if (someRowsSelected) {
            var selectedCompanyId = null;
            selectedRows.each(function() {
                var rowData = table.row($(this)).data();
                selectedCompanyId = rowData.IdCompany;
            });
            groupRows.each(function() {
                var rowData = table.row($(this)).data();
                if (rowData.IdCompany === selectedCompanyId) {
    ...
    // Enable all checkboxes if no row or group-checkbox is selected
    if (!anyRowSelected) {
        $('.group-checkbox').prop('disabled', false);
        $('#QueueListTable tbody tr').find('input[type="checkbox"]').prop('disabled', false);
    }
}

// Function to disable group-checkboxes when there are multiple companies under each group
function disableCheckboxMultipleCompanies(table) {
    var groups = $('.group-checkbox');
    ...
    groups.each(function() {
        ...
        var groupRows = $('#QueueListTable tbody tr').filter(function() {
            var rowData = table.row($(this)).data();
            return rowData && rowData.NameDoctor === groupName;
        });

        var companyNames = [];
        groupRows.each(function() {
            var rowData = table.row($(this)).data();
            var companyName = rowData.NameCompany;
...
// Function to remove group-checkboxes when the balance of rows under each group is 0
function removeCheckboxDoctorFullyPaid(table) {
    var groups = $('.group-checkbox');
    ...
    groups.each(function() {
        ...
        groupRows.each(function() {
            var rowData = table.row($(this)).data();
            balance += parseFloat(rowData.AmountRemaining);
        });

        if (balance === 0) {
            groupCheckbox.remove();
        }
    });
}
...
$(document).ready(function(e) 
{
	...
	$html += "<thead>";
          $html += "<tr>";
	$html += "<th></th>";
	$html += "<th class=\"text-center\"><input name=\"select_all\" id=\"select_all\" value=\"1\" type=\"checkbox\" hidden></th>";
	...
	var table = $('#QueueListTable').DataTable({
		...
		columnDefs		: [
			...
			{ data: null, targets: 1, className:'sgroup', orderable: false,"width":"10px" },
	...
	$('#QueueListTable').dataTable().rowGrouping({
			...
			fnGroupLabelFormat: function (doctor) {
			return '<input type="checkbox" class="group-checkbox" value="'+ doctor + '" onclick="checkDoctor(this)">&nbsp;' + doctor;
		}
	});
	
	// FOR CLEARING SELECTED on GROUP CHECKBOX IF SOME ROWS ARE SELECTED 02/04/2024
	$('.group-checkbox').click(function(event) {
		var groupCheckbox = $(this);
		if (groupCheckbox.prop('indeterminate')) {
			// Reset the state of checkboxes within the group
			var groupName = groupCheckbox.val();
			var groupRows = $('#QueueListTable tbody tr').filter(function() {
				var rowData = table.row($(this)).data();
				return rowData && rowData.NameDoctor === groupName && (rowData.Status === 201 || rowData.Status === 202);
			});

			groupRows.find('input[type="checkbox"]').prop('checked', true).prop('disabled', false);
			groupRows.find('input[type="checkbox"]').trigger('click');
			groupCheckbox.prop('indeterminate', false);
		}
	...
	disableCheckboxMultipleCompanies(table);
	removeCheckboxDoctorFullyPaid(table);
...
$('#QueueListTable tbody').on('click', 'input[type="checkbox"]', function(e){
	...
	checkGroupCheckboxStatus(table);
   ...
   $('#QueueListTable').on('click', 'tbody td', function(e){
	...
	if($(this).find('.group-checkbox').length > 0) return;
