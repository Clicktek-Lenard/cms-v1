...
<div class="row form-group row-md-flex-center" id="addPhysician" >
			...
			<div class="col-sm-3 col-md-3">
				<input type="text"  id="prcNo" class="typeahead form-control" name="prcNo" placeholder="PRC NO."  pattern="\d*" data-toggle="tooltip" title="Please enter only digits">
			</div>
			<div class="col-sm-3 col-md-3">
            ...
            action: function (modalRef) {
                ...
                if (selectedRow.length > 0) {
                    ...
                    if (status === "RP - Leads" || status === "RP - For Approval" || status === "Active") {
                        ...
                        // Only set the value if selectedId is truthy
                        if (selectedId) {
	...
	buttons: [
		...
		},
		{
			...
			action: function (modalRef){
				...
				if (!lastname || !firstname || !prcno || !myimage) {
					...
					data: { 
						...
						success: function(response) {
							...
							if (response.exists) {
								...
								alert('New physician inserted successfully. ID: ' + response.id);
							}
...
$(document).ready(function(e) {	
	...
	var table = $('#ItemListTable').DataTable({
		...
		'rowCallback': function(row, data, dataIndex){
			...
			rows_selected = $('.rows_selected').html();
			...
			else
			{
				parent.getData(
					...
					function(results){
						...
						$('.rows_selected').html(rows_selected);
						itemTable.fnClearTable();
