        ...
        <div class="col-menu-10">
            <div class="header-crumb">
                <ol class="breadcrumb" style=" border-radius:0; " >
                    <li class="active"><a href="{{ '/specimen-receiving/bloodextraction' }}" class="waiting">Blood Extraction <span class="badge" style="top:-9px; position:relative;"></span></a></li>
                </ol>
            </div>
        </div>
    </div>
...
$(document).ready(function(e)
{
    ...
    $searchInput.on('input', function() {
		...
		$('#QueueListTable').on('click','.data-row',function(e){
            ...
            sendout.setData('pageToLoad', '/specimen-receiving/specimen/' + id + '/edit?subgroup=HEMATOLOGY,CHEMISTRY,IMMUNOLOGY');
            sendout.setData('queueId', id); // Store queueId in dialog data
            sendout.setData('subgroup', 'HEMATOLOGY');
        ...
        var sendout = new BootstrapDialog({
            ...
            },
            {
                ...
                label: 'Select Tubes',
                action: function (modalRef) {
                    console.log("Select Tubes Success clicked", window.checkedRowsData);
                }
            },
            {
                id: 'btnsave',
                ...
                action: function (modalRef) {
                    ...
                    if (window.checkedRowsData && window.checkedRowsData.length > 0) {
                        let invalidRows = window.checkedRowsData.filter(r => {
                            if (r.Waived === 1) return false;
                            if (r.DoneOutside === 1) return false;
                            if (!r.Tubes) return true; // no Tubes object
                            // check if all tube counts are 0
                            return Object.values(r.Tubes).every(v => v === "0" || v === 0);
                        });

                        if (invalidRows.length > 0) {
                            Swal.fire({
                                icon: 'warning',
                                title: 'No Tubes Selected!',
                                text: 'Please Select Tubes Before Saving!',
                                customClass: { 
                        ...
                        $.ajax({
                            url: `/specimen-receiving/specimen/${queueId}/receive?subgroup=HEMATOLOGY,CHEMISTRY,IMMUNOLOGY&type=bloodextraction`,
                            method: 'POST',
                            data: {
